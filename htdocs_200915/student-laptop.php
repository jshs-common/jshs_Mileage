...
<html>
    ...
    if(IsCookieSet() && CookieLogin($connect))
    {
        ...
        $RetQuery = "SELECT ban, BanDay FROM laptop_return WHERE SID=".$SID.";";
        $return = mysqli_query($connect, $RetQuery);
        $row2 = mysqli_fetch_array($return);
        $week=array("일", "월", "화", "수", "목", "금", "토");
        $WeekDay=$week[date('w', strtotime($row2['BanDay']))];

        if(isset($row) && ($row['IsApproved'] != 1))
        {
          echo "<meta http-equiv='refresh' content='0;url=laptop-waiting.php'>";
          exit;
        }
        elseif($row['IsApproved'] == 1) {
            $query2 = "SELECT due FROM apply_laptop WHERE SID=".$SID.";";
            $result2 = mysqli_query($connect, $query2);
            $row = mysqli_fetch_array($result2);
            $day = (int)$row['due'];
            if($day<0) {
                echo "<script>alert(\"이미 신청되어 있습니다. ".$day*(-1)."일 연체됐습니다.\"); document.location.href='choose.php'</script>";
            }
            else {
                echo "<script>alert(\"이미 신청되어 있습니다. 반납까지 ".$day."일 남았습니다.\"); document.location.href='choose.php'</script>";
            }
            
            exit;
        }
        elseif($row2['ban']>0) {
            echo "<script>alert(\"연체일수가 누적되어 노트북을 대여할 수 없습니다.\\n".$row2['BanDay']."(".$WeekDay.")"."까지 노트북을 대여할 수 없습니다.\"); document.location.href='choose.php'</script>";
            exit;
        }
    }
    ?>
        ...
        <div class="container">
            <div class="row" style="margin-top:2%;">
                ...
                <div class="col-xs-10 col-sm-6">
                    <h1><?php echo $_COOKIE['UserName']; ?>님의 노트북 신청</h1>
                </div>
            </div>
            <div class="row" style="margin-top:2%;">
                ...
                <div class="col-sm-offset-1 col-sm-7" style="max-height:60vh;" id="box2">
                    ...
                    <div id="result" style="max-height:40vh; overflow-x:hidden; overflow-y:scroll;">
                        <h6>
                            <?php
                                if($subject == "all"){
                                    $query = "SELECT * FROM laptop_list WHERE num != 0";
                                }
                                elseif($num == "all"){
                                ...
                                echo "<table class='table table-striped table-bordered table-hover, display:inline-block'>
                                    <tr>
                                        ...
                                        <th style='text-align:center;'>대여여부/신청</th>
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    ...
                                    if($row[2] == 1){
                                        ...
                                        if($row[3] == 2){
                                            echo "<td style='text-align:center; color:red;'>대여중</td>";
                                        }
                                        elseif ($row[3] == 1) {
                                            ...
                                            <form action='student-laptop-apply.php' method='post'>
                                            <input type='hidden' name='nb-subj' value='".$row[0]."'>
                                            <input type='hidden' name='nb-num' value='".$row[1]."'>
                                            <button type='submit' class='btn btn-success' style='font-size:12px; margin:0px; border:0px; padding:3px 15px;'>노트북 대여 신청</button>
                                            </form>
                                            </td>";
                                        }
                                    }
