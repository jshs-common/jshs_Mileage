<?php
  ...
  if($_COOKIE['UserSID'] == 319||$_COOKIE['UserSID'] == 310) {
    if(IsCookieSet() && CookieLogin($connect)) {
      ...
      $borrow = $_POST['borrow'];
      $cannotborrow = $_POST['cannotborrow'];
    }
    else{
        error(2);
        exit;
  ...
  if($cannotborrow == -1){
    $connect->query("update laptop_list set trouble = 1 where SubjNum =".$SubjNum.";");
  }else if($cannotborrow == 1){
    $connect->query("update laptop_list set borrow = 2 where SubjNum =".$SubjNum.";");
  }else if($cannotborrow == 2){
    $connect->query("update laptop_list set trouble = 0 where SubjNum =".$SubjNum.";");
  }
