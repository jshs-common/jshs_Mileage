            ...
            <div class="li-inside">
              <!-- 수정중인 부분! -->
                  <select id="select_grade">
                    <option>학년 선택</option><option>1학년</option><option>2학년</option><option>3학년</option>
                  </select>

                    <select id="select_type">
                      <option>반 선택</option><option>1반</option><option>2반</option>
                    </select>
                  ...
                  <button onclick="javascript: chkIndex();">테스트</button>
                </div>
                <script src="http://ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js"></script>
                <script type="text/javascript"> 
                  var name_11 = [["이름 선택"],"11", "2", "3","4","5"];
                  var name_12 = [["이름 선택"],"12", "2", "3","4","5"];
                  var name_21 = [["이름 선택"],"13", "2", "3","4","5"];
                  var name_22 = [["이름 선택"],"14", "2", "3","4","5"];
                  var name_31 = [["이름 선택"],"15", "2", "3","4","5"];
                  var name_32 = [["이름 선택"],"16", "2", "3","4","5"];
                  var sel_cls= [ ["이름 선택"], name_11, name_12, name_21, name_22, name_31, name_32];
                  function createTag(index) {
                      var result = "";
                      sel_cls[index].forEach(function (item) {
                          result += "<option>" + item + "</option>";
                      });
                      return result;
                  }
                  function chgOptions() {
                          var selected_index = $("#select_grade option").index($("#select_grade option:selected"))*2-$("#select_type option").index($("#select_type option:selected"))%2;
                          $("#select_name").html(createTag(selected_index));
                      }
...
<!-- 넘버링 -->
               <script type="text/javascript">
                  function chkIndex() {
                    alert($("#select_grade option").index($("#select_grade option:selected")));
                    alert($("#select_type option").index($("#select_type option:selected")));
                    alert($("#select_name option").index($("#select_name option:selected")));
                    alert($("#select_grade option").index($("#select_grade option:selected"))*1000+$("#select_type option").index($("#select_type option:selected"))*100+$("#select_name option").index($("#select_name option:selected")));
                  }
               </script>
<!-- 
                <input type="hidden" name="student" placeholder="(설명-대충 이름의 변수 값)" class="form-control" maxlength="100" onkeypress="nodotnspace();" required>
 -->
              </div>
            </div>
          </li>
