...
<?php
  ...
  if(CookieLogin($connect))
  {
    if(isset($_POST['lastpw']) && isset($_POST['newpw']) && isset($_POST['newpwcheck']))
    {
      $id = $_COOKIE['UserName'];
      $realid = explode( '(', $id);
      $pw = $connect->query("select Password from user where ID = '".$realid[0]."';")->fetch_array()['Password'];
      $lastpw = $_POST['lastpw'];
      $newpw = $_POST['newpw'];
      ...
      if(strcmp($pw, $lastpw))
      {
        ...
        $connect->query("update user set Password='".$newpw."' where ID='".$realid[0]."';");
        echo "
        <script>
    ...
    <div class="row">
      ...
      <div class="col-sm-offset-2 col-sm-8 col-md-offset-4 col-md-4 login-card">
          ...
          <form action="changepw.php" method="post" enctype="multipart/form-data">
              ...
              <div class="group">
                  <input type="password" name="newpw" maxlength="20" required />
                  <span class="bar"></span>
                  <label><span class="input-label">새로운 비밀번호</span></label>
              </div>
          
              <div class="group">
                   <input type="password" name="newpwcheck" maxlength="20" required />
                   <span class="bar"></span>
