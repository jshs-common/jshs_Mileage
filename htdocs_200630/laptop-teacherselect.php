...
<?php
  ...
  $postvalue = array();
  $postname = array();
  array_push($postvalue, $_POST['laptop'], $_POST['lenddate'], $_POST['reason'], $_POST['addition']);
  array_push($postname, 'laptop', 'lenddate', 'reason', 'addition');

  //$result = $connect->query('select Attendance from teachers where SID = 319') or die($this->_connect->error);
  $name = array();
  $SID = array();
  array_push($name, "진우용", "양은심");
  array_push($SID, 319, 310);
...
<script>
  ...
  function goPost() {
    ...
    form.setAttribute("action", "laptop-finish.php");
